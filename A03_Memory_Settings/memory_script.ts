//Habe wie immer mit Dino zusammengearbeitet! :)

namespace L03Memory_Settings {
    ...
    start.addEventListener("click", createGameCards);
    }
    

    function createGameCards(_event: Event): void {  
    ...
    function endGame(): void {
        if (hideCards.length == playCardArray.length) {
            ...
            let restart: HTMLElement = document.createElement("div");
            restart.innerHTML = "<button>Neustart</button>";
            restart.classList.add("Neustart");
            div.appendChild(restart);
            restart.addEventListener("click", function(e: Event): void {
            e.preventDefault();
            location.reload();
            });
        }
